...
import type { EditCmcModalProps } from '../types';


export function EditCmcModal({ isOpen, onClose, onUpdate, cmc }: EditCmcModalProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    const result = await onUpdate(cmc.id, formData);
    setLoading(false);

    if (result.success) {
      onClose();
    } else {
      setError(result.error || 'Failed to update CMC');
    }
  };
