...
import { type AddCmcModalProps } from '../types';

const API_BASE_URL = import.meta.env.API_BASE_URL || 'http://localhost:3001';

export const AddCmcModal = ({ onAdd, onClose }: AddCmcModalProps) => {
  ...
  const handleChange = (field: string, value: string) => {
    ...
    if (errors[field]) setErrors(prev => ({ ...prev, [field]: undefined } as { [key: string]: string }));
          ...
          <div>
            ...
            {testMessage && (
              <div role="alert" className={`alert mt-3 text-sm ${
                ...
                {testStatus === 'success' && <CheckCircle className="w-4 h-4 shrink-0" />}
