...
function AppContent() {
  ...
  const [editingCmc, setEditingCmc] = useState<Cmc | null>(null);
  ...
  const handleAddCmc = async (cmcData: CmcPayload): Promise<ActionResult> => {
  ...
  const handleEditCmc = async (id: string, cmcData: CmcPayload): Promise<ActionResult> => {
  ...
  const handleDeleteCmc = async (id: string) => {
    ...
    <div className="h-screen flex flex-col bg-base-200 ">
      ...
      <header className="bg-base-100 border-b border-base-300 px-4 py-3 w-full">
        <div className="navbar flex flex-none py-0 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* User Info */}
            <div className="btn-sm mx-2 px-3 flex items-center gap-2 py-2 bg-base-200 rounded-lg">
              {isAdmin() ? (
      ...
      {showAddModal && (
        <AddCmcModal
          ...
          onAdd={async (data) => await handleAddCmc(data)}
        />
      )}
